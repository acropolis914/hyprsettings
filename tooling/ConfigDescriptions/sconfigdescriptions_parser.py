...
file_path = Path(f'{Path(__file__).resolve().parent}/ConfigDescriptions0.54.hpp')
...
for obj in config_list:
	...
	if obj['data']:
		...
		if data == 'STRVAL_EMPTY':
			obj['data'] = ''
		else:
			data = data.replace('\\"', '')
			data = re.sub(r'\.(?:value|min|max)\s*=\s*', '', data)
			obj['data'] = data

# jsonstring = json.dumps(config_list, indent=4)
# print(jsonstring)
with open(f'{Path(__file__).resolve().parent}/configDescriptions.ts', 'w+', encoding='utf-8') as description_file:
	description_file.write("""import type { ConfigDescription } from '@scripts/types/configDescriptionTypes.ts'

export const HyprConfigDescriptions: ConfigDescription[] = """)
	description_file.write(helpers.dumpjsobject(config_list))
	description_file.write(';')
	description_file.write(f'\n// Autogenerated from {basename}\n')

shutil.move(
	  f'{Path(__file__).parent.resolve()}/configDescriptions.ts',
	  f'{Path(__file__).parent.parent.parent / "src" / "ui-src" / "scripts" / "HyprlandSpecific" / "hyprConfigDescriptions.ts"}',
)

# with open(f'{Path(__file__).resolve().parent}/cd.json', 'w+', encoding='utf-8') as description_file_json:
# 	json.dump(config_list, description_file_json, indent=2, ensure_ascii=False)
# 	subprocess.Popen(['kitty', '--hold', 'jless', f'{Path(__file__).resolve().parent}/cd.json'])
...
# then fix the data on the output js
#     "name": "vrr",
#     "path": "misc",
#     "type": "CONFIG_OPTION_INT",
