...
They can be enabled in the `background-effect {}` section of [window](./Configuration:-Window-Rules#background-effect) or [layer](./Configuration:-Layer-Rules#background-effect) rules.
...
Focus ring and border can also cover the background effect, see [this FAQ entry](./FAQ#why-are-transparent-windows-tinted-why-is-the-borderfocus-ring-showing-up-through-semitransparent-windows) for how to change this.
...
Blur enabled via the window rule will follow the window corner radius set via [`geometry-corner-radius`](./Configuration:-Window-Rules#geometry-corner-radius).
...
On the niri side, you can do it with a `popups` block inside [`window-rule`](./Configuration:-Window-Rules#popups) and [`layer-rule`](./Configuration:-Layer-Rules#popups).
See those wiki pages for examples and limitations.

Global blur settings are configured in the [`blur {}` config section](./Configuration:-Miscellaneous#blur) and apply to all background blur.

### Xray

Xray makes the window background "see through" to your wallpaper, ignoring any other windows below.
You can enable it with `xray true` background effect [window](./Configuration:-Window-Rules#background-effect) or [layer](./Configuration:-Layer-Rules#background-effect) rule.
...
The `ext-background-effect` protocol supports any wl\_surface.
