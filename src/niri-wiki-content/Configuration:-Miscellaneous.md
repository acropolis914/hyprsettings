...
See the [window effects page](./Window-Effects) for an overview of background effects.
...
You can also enable it manually with the `blur true` background effect [window](./Configuration:-Window-Rules#background-effect) or [layer](./Configuration:-Layer-Rules#background-effect) rule.
