		...
		while self.char is not None:
			try:
				if self.char == 'r' and self.peek(1) == '#' and self.peek(2) == '"':
					...
					newNode = KDLToken('REGX', raw_string, position=self.pos)
					...
					newNode = KDLToken(token_type, num, position=self.pos)
					self.tokens.append(newNode)
					continue
				elif self.char.isalpha():
					word = self.gather_word()
					if word == 'true' or word == 'false':
						newNode = KDLToken('BOOL', word, position=self.pos)
					else:
						newNode = KDLToken('WORD', word, position=self.pos)
					self.tokens.append(newNode)
					continue
					...
					newNode = KDLToken('SLASHDASH', '/-', position=self.pos)
					self.tokens.append(newNode)
					self.advance(2)
					continue
				elif self.char == '/' and self.peek(1) == '*':
					comment_block = self.gather_comment_block()
					newNode = KDLToken('COMMENTBL', comment_block, position=self.pos)
					self.tokens.append(newNode)
					self.advance(2)
					...
					newNode = KDLToken('STRING', raw_string, position=self.pos)
					self.advance()
					self.tokens.append(newNode)
					continue
				elif self.char == '\n':
					newNode = KDLToken('BR', self.char, position=self.pos)
					self.tokens.append(newNode)
					self.advance()
					continue
					...
					while self.peek().isspace():
						self.advance()
					newNode = KDLToken('WS', self.char, position=self.pos)
					self.tokens.append(newNode)
					self.advance()
					continue
				elif self.char == '{':
					newNode = KDLToken('LBRACE', self.char, position=self.pos)
					self.tokens.append(newNode)
					self.advance()
					continue
				elif self.char == '}':
					newNode = KDLToken('RBRACE', self.char, position=self.pos)
					self.tokens.append(newNode)
					self.advance()
					continue
				elif self.char == ';':
					newNode = KDLToken('SEMIC', self.char, position=self.pos)
					self.tokens.append(newNode)
					self.advance()
					continue
				elif self.char in '+=-*':
					newNode = KDLToken('OPERATION', self.char, position=self.pos)
					self.tokens.append(newNode)
					self.advance()
					continue
				elif self.char is not None:
					newNode = KDLToken('UNKNOWN', self.char, position=self.pos)
					self.tokens.append(newNode)
					self.advance()
					continue
				else:
