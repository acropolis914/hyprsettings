...
def createArgParse(parser):
	...
	general.add_argument('-k', '--kill', action='store_true',
	                     help='Use this arg to kill any running hyprsettings process.')
	general.add_argument('-N', '--no-daemon', action='store_true',
	...
	ui.add_argument('-u', '--ui', choices=['gtk', 'qt', 'electron'], default='gtk',
	                help='Run the UI with specified backend (qt requires qt library).')
	ui.add_argument('-H', '--hidden', action='store_true',
	                help='Start hidden. Useful for autostarting with daemon (e.g., in hyprland.conf).')
	ui.add_argument('-b', '--no-window', action='store_true',
	...
	dev.add_argument(
		  '--bun-dev', action='store_true',
		  help='[Deprecated] Use the Bun development server (localhost:3000). WARNING: FOR DEVELOPMENT ONLY.'
	)
	# Configuration
