...
!(function (e, t) {
	...
	const O = (e) => {
		...
		C = (e, t) => {
			for (const s of t) {
				...
				const i = (e) => {
					for (const s of t) {
						if (s.start === e.start && s.substr === e.substr)
							return !1
						if (1 != e.length && 1 != s.length) {
							if (e.start < s.start && e.end > s.start)
								return !0
							if (s.start < e.start && s.end > e.start)
								return !0
						}
					}
					return !1
	...
	class T {
		...
		_getSortFunction(e) {
			...
			const r = function (t, s) {
				return '$score' === t
					? s.score
					: e.getAttrFn(i.items[s.id], t)
			}
			if (o)
				for (let t of o)
					(e.query || '$score' !== t.field) && s.push(t)
			if (e.query) {
				t = !0
				for (let e of s)
					if ('$score' === e.field) {
	...
	const P = (e) => (null == e ? null : N(e)),
		...
		te = (e, t) => {
			if (!e) return -1
			t = t || e.nodeName
			for (var s = 0; (e = e.previousElementSibling); )
				e.matches(t) && s++
			return s
		},
		se = (e, t) => {
		...
		return (
			'select' === d
				? (() => {
						...
						;((p.maxItems = e.hasAttribute('multiple')
							? null
							: 1),
							B(e.children, (e) => {
								var s, i, n
	...
	class ce extends (function (e) {
		...
		constructor(e, t) {
			...
			;((this.settings = n),
				...
				(n.mode =
					n.mode || (1 === n.maxItems ? 'single' : 'multi')),
		...
		setup() {
			...
			if (g) {
				...
				;(se(c, { 'aria-labelledby': t }),
					se(n, { 'aria-labelledby': t }))
			}
		...
		setTextboxValue(e = '') {
			...
			t.value !== e &&
				((t.value = e), G(t, 'update'), (this.lastValue = e))
		}
		getValue() {
		...
		updateOption(e, t) {
			...
			if (
				...
				if (s.dropdown_content.contains(a)) {
					...
					;(ie(a, e),
						s.activeOption === a && s.setActiveOption(e))
				}
				a.remove()
			}
			;(c &&
		...
		addItems(e, t) {
			...
			const n = (i = i.filter((e) => -1 === s.items.indexOf(e)))[
				i.length - 1
			]
			i.forEach((e) => {
				...
				if (
					...
					if (
						...
						;(n.isPending ||
							...
							n.refreshOptions(
								n.isFocused && 'single' !== o,
							),
							0 != n.settings.closeAfterSelect &&
							n.isFull()
								? n.close()
		...
		deleteSelection(e) {
			...
			;((t = e && 8 === e.keyCode ? -1 : 1),
				(s = {
					...
					length:
						(o.selectionEnd || 0) - (o.selectionStart || 0),
				}))
			const l = []
		...
		hook(e, t, s) {
			...
			i[t] = function () {
				...
				return (
					...
					'instead' === e
						? o
						: ('before' === e && (t = n.apply(i, arguments)),
							t)
				)
			}
		}
	...
	return (
					...
					return (
						...
						D(r, 'dragend', () => {
							...
							;(document
								...
								.forEach((e) =>
									e.classList.remove('ts-drag-over'),
								),
								null == (t = i) ||
									t.classList.remove('ts-dragging'),
								(i = void 0))
							var s = []
							;(e.control
		...
		ce.define('caret_position', function () {
			...
			;(e.hook('instead', 'setCaret', (t) => {
				...
				e.control.contains(e.control_input)
					? (t = Math.max(0, Math.min(e.items.length, t))) ==
							e.caretPos ||
						e.isPending ||
						e.controlChildren().forEach((s, i) => {
							i < t
								? e.control_input.insertAdjacentElement(
										'beforebegin',
										s,
									)
			...
			;((e.settings.shouldOpen = !0),
				e.hook('before', 'setup', () => {
					;((e.focus_node = e.control),
						J(e.control_input, 'dropdown-input'))
					...
					;((s.placeholder = e.settings.placeholder || ''),
						e.control.append(s))
				}),
				e.on('initialize', () => {
					;(e.control_input.addEventListener('keydown', (t) => {
						...
						e.on('blur', () => {
							e.focus_node.tabIndex = e.isDisabled
								? -1
								: e.tabIndex
						}),
						e.on('dropdown_open', () => {
					...
					;(e.hook('instead', 'onBlur', (s) => {
						if (!s || s.relatedTarget != e.control_input)
							return t.call(e)
					}),
		...
		ce.define('virtual_scroll', function () {
				...
				(e.settings.shouldLoadMore ||
					(e.settings.shouldLoadMore = () => {
						...
						if (e.activeOption) {
							var t = e.selectable()
							if (
								Array.from(t).indexOf(e.activeOption) >=
								t.length - 2
							)
								return !0
						}
			...
			;((e.setNextUrl = (e, t) => {
				...
				e.hook('instead', 'canLoad', (s) =>
					s in r ? c(s) : t.call(e, s),
				),
				...
				e.hook('after', 'refreshOptions', () => {
					...
					;(c(t)
						? (s = e.render('loading_more', { query: t })) &&
							(s.setAttribute('data-selectable', ''),
							(o = s))
						: t in r &&
