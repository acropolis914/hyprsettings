...
var documentExists = typeof document !== 'undefined',
	...
	_detectDirection = function _detectDirection(el, options) {
		var elCSS = css(el),
			...
			firstChildWidth =
				firstChildCSS && parseInt(firstChildCSS.marginLeft) + parseInt(firstChildCSS.marginRight) + getRect(child1).width,
			secondChildWidth =
...
Sortable.prototype = /** @lends Sortable.prototype */ {
	...
	_onTapStart: function _onTapStart(/** Event|TouchEvent */ evt) {
		...
		var _this = this,
			...
			originalTarget =
				(evt.target.shadowRoot && ((evt.path && evt.path[0]) || (evt.composedPath && evt.composedPath()[0]))) || target,
...
var autoScroll = throttle(function (evt, options, rootEl, isFallback) {
	...
	do {
		...
		if (el === winScroller) {
			canScrollX =
				width < scrollWidth && (elCSS.overflowX === 'auto' || elCSS.overflowX === 'scroll' || elCSS.overflowX === 'visible')
			canScrollY =
				height < scrollHeight && (elCSS.overflowY === 'auto' || elCSS.overflowY === 'scroll' || elCSS.overflowY === 'visible')
		} else {
		...
		var vy =
			canScrollY &&
			(Math.abs(bottom - y) <= sens && scrollPosY + height < scrollHeight) - (Math.abs(top - y) <= sens && !!scrollPosY)
