		...
		if (touchEvt) {
			...
			if (parent) {
				do {
					...
					/* jshint boss:true */ (parent =
						getParentOrHost(parent))
				)
			}
		...
		this.defaults = {
			...
			setData: function setData(dataTransfer, dragEl) {
				...
				if (
					...
					multiDragElements.forEach(
						function (multiDragElement, i) {
							data +=
								(!i ? '' : ', ') +
								multiDragElement.textContent
						},
					)
				} else {
				...
				if (
					...
					if (!initialFolding) {
						if (options.animation) {
							...
							multiDragElements.forEach(
								function (multiDragElement) {
									multiDragElement.thisAnimationDuration =
										null
									if (
										multiDragElement !== dragEl$1
									) {
										var rect = folding
											? getRect(
													multiDragElement,
