	...
	return (tree) => {
		visit(tree, (node) => {
			...
			if (node.type === 'blockquote' && node.children.length > 0) {
				...
				if (first.type === 'paragraph' && first.children.length > 0 && first.children[0].type === 'text') {
					...
					if (match) {
						...
						data.hName = 'div'
						data.hProperties = {
							className: ['admonition', type, 'blockquote'],
						}

						// Remove the [!TYPE] marker from the first paragraph
						first.children[0].value = first.children[0].value.replace(/^\[!(NOTE|TIP|WARNING|CAUTION)]\s*/, '')
					}
				}
			}
		})
