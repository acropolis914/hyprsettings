...
class Node:
    ...
    def to_hyprland(
        self, indent_level: int = 0, save=False, changedFiles: list = [], disabled=False
    ) -> list | str:
        ...
        if self.type == "KEY":
            ...
            if self.disabled or disabled:
                log(
                    f"Key {self.name} is disabled by {'itself' if self.disabled else 'parent'}."
                )
        ...
        if self.children:
            ...
            if self.type == "GROUP" and self.name != "root":
                ui_print(
                    f"Proc. group {self.name} which is disabled: {self.disabled or disabled}"
                )
                group_content: list = []
                comment = f" # {self.comment}" if self.comment else ""
                disabled_text = "#DISABLED " if (self.disabled or disabled) else ""
                group_content.append(
                    f"{indent * indent_level}{disabled_text}{self.name}"
                    + " {"
                    + comment
                )
                indent_level += 1
        ...
        with open(config_path, "r", encoding="UTF-8") as config_file:
            ...
            for line_index, line_content in enumerate(config_file, start=1):
                ...
                # 	pass
                    ...
                    ui_print(
                        f"Line {line_index} is unrecognized: {line_content.strip()}"
                    )
                    # name, value = self.get_parts(line, "=")
                    # if value is None:
