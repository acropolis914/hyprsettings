...
dev-niri:
    cd src/ui-src && bunx concurrently -k -s first -n py,vt -c cyan,green "sh ../../run.sh --bun-dev --debug --no-daemon --no-browser --ui electron --config config_niri_short.kdl" "vite --host"
